			...
			if assert.Nil(t, err, "Error creating cloud info service") {
				...
				if assert.Nil(t, err, "Error getting cluster config path") {
					...
					for secretName, secretNamespace := range secretsMap {
						ocOptions := k8s.NewKubectlOptionsContext(context.Background(), "", clusterConfigPath, secretNamespace)
			...
			if assert.Nil(t, err, "Error creating cloud info service") {
				...
				if assert.Nil(t, err, "Error getting cluster config path") {
					...
					if assert.Nil(t, err, "Error reading variables file") {
						...
						reloaderPods, err := k8s.ListPodsContextE(context.Background(), t, k8s.NewKubectlOptionsContext(context.Background(), "", clusterConfigPath, esoNamespace), metav1.ListOptions{
					...
					ocOptions := k8s.NewKubectlOptionsContext(context.Background(), "", clusterConfigPath, namespace)
					// deploy sample app

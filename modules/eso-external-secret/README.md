...
| <a name="input_sm_service_credentials_mappings"></a> [sm\_service\_credentials\_mappings](#input\_sm\_service\_credentials\_mappings) | Map of Kubernetes secret key to ESO template expression. If empty, the complete service credential JSON is stored using es\_kubernetes\_secret\_data\_key. | `map(string)` | `{}` | no |
